...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  const login = useCallback(async (email: string, password: string): Promise<boolean> => {
    ...
    setError(null);
    try {
      const response = await fetch('/.netlify/functions/login', { // URL de la fonction Netlify
        method: 'POST',
        headers: {
  ...
  const signup = useCallback(async (nom: string, prenom: string, email: string, password: string): Promise<boolean> => {
    ...
    setError(null);
    try {
      const response = await fetch('/.netlify/functions/signup', { // URL de la fonction Netlify
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
