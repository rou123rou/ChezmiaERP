        ...
        <nav className={`fixed top-0 left-0 right-0 z-50 bg-gradient-to-r from-orange-100 to-yellow-100 shadow-md py-3 md:flex`}>
            <div className="container mx-auto px-4 flex items-center justify-between">
                ...
                <ul className="flex space-x-6 items-center">
                    ...
                    {isAuthenticated && (
                        <>
                            <li><Link to="/profil" className="hover:text-blue-500">Mon Profil</Link></li>
                            <li><button onClick={handleLogout} className="hover:text-red-500 focus:outline-none">Déconnexion</button></li>
                            {user?.nom && <li><span className="text-gray-700">Bonjour, {user.nom}</span></li>}
                            <li className="relative hover:text-blue-500 focus:outline-none">
                                <span
                                    ref={cartLinkRef}
